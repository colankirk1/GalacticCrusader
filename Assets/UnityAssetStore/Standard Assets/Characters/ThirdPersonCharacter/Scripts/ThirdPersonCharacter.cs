		...
		public void Move(Vector3 move, bool crouch, bool jump)
		{
			...
			if(m_ForwardAmount<0.0){
				...
				if(turnSpeed == 0) {
					...
					float turnSpeed2 = Mathf.Lerp(m_StationaryTurnSpeed, m_MovingTurnSpeed, m_ForwardAmount);
					transform.Rotate(0, m_TurnAmount * turnSpeed2 * Time.deltaTime, 0);
				}
				//transform.Rotate(0.0F,  turnSpeed, 0.0F);
		...
		void CheckGroundStatus()
		{
			...
			if (Physics.Raycast(transform.position + (Vector3.up * 0.1f), Vector3.down, out hitInfo, m_GroundCheckDistance))
			{
				if(hitInfo.transform.tag == "Exit") {
					int tempNum = thisLevelNum + 1;
					int numPurp = PlayerPrefs.GetInt("numPur", 0); 
					if(tempNum > 2 && numPurp == 0) {
						tempNum = 2;
					} else if(tempNum > 3 && numPurp > 0) {
						tempNum = 3;
					}
					PlayerPrefs.SetInt("unlockLevel", tempNum);  
					PlayerPrefs.Save ();
					hitInfo.transform.GetComponent<ExitLevelController>().loadThatLevel();
				}
